<div class="filter-content card-container ">
    @foreach($filter_by_engement_rate as $key=>$value)
        <input type="hidden" id="max_rate" value="{{ $max_rate }}"/>
        <input type="hidden" id="min_rate" value="{{ $min_rate }}"/>
    <div class="card-list">
...
@if(count($filter_by_engement_rate)>0)
