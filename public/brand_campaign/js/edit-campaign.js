...
----------------------------------------------------------------------------------- */
  ...
  function getDataFromTheEditor() {
    ...
    $('.btn-next').on('click', function() {
        ...
        if(currentStepNum == 1){
            ...
            $('#amount, #total_influencers_required, #budget_for_each_influencer, #application_till_date, #application_start_date, #application_end_date, #traceable_link').next('p').addClass('d-none');

            if(!$(":checkbox[name='is_sample_required']", $(".edit_campaign_form")).is(":checked")) {
                isAjax = false;
            ...
            if(document.getElementById("image").files.length != 0 && $.inArray(document.getElementById("image").files[0].type.split('/').pop().toLowerCase(), fileExtension) == -1){
                isAjax = false;
                error_notification_add('Select only jpg, jpeg and png files!');
                return false;
            }
            if(!$(":checkbox[name='is_video']", $(".edit_campaign_form")).is(":checked")) {
                isAjax = false;
                scrollTop();
            ...
            if(!$(":checkbox[name='campaign_is_active']", $(".edit_campaign_form")).is(":checked")) {
                isAjax = false;
  ...
  $('.btn-submit').on('click',function(){
      ...
      $('.error-product').addClass('d-none');
      if(sysUserCount == 0 && scrapUserCount == 0 && scrapInfArr.length <= 0 && normalInfArr.length <= 0) {
          isAjax = false;
  ...
  function ajaxCall(isAjax){
      if(isAjax){
        ...
        $('.loading').removeClass('d-none');
          var formData = new FormData($('.edit_campaign_form')[0]);
          $.ajax({
              url: update_route,
              type: 'POST',
              dataType: 'json',
            ...
            if (response.status) {
                ...
                if(response.status && response.products.length > 0){
                    $.each(response.products, function(key, value) {
                        var isSelected = $.inArray(value.product_id,oldProdIdArr) == -1 ? '' : 'selected';
                        $('#select_products').append(`<option value="${value.product_id}" data-img_url="${value.product_img}" ${isSelected}>${value.product_name}</option>`);
                    });
...
function removeInfluancer(campaignId, influencerId, autoIncId){
    if(campaignId && influencerId){
        $.ajax({
            url: delete_inf,
            type: 'POST',
            dataType: 'json',
            data: { 'campaign_id': campaignId, 'influencer_id' : influencerId },
            success: function (response) {
                if (response.status) {
                    delete_notification(response.message);
                    $(`.li-section-${autoIncId}`).remove();
                } else {
                    error_notification(response.message);
                }
            },
        ...
        error_notification('Campaign and Influanecer id is necessary!');
    }
}
